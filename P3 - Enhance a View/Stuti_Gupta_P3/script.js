...
function renderCharts(data) {
  ...
  // Cuisine Pie Chart
  const ctx1 = document.getElementById("cuisineChart");
  if (ctx1) {
    new Chart(ctx1, {
      type: "pie",
      data: {
        labels: sortedCuisines.map(([c]) => c),
        datasets: [{
          data: sortedCuisines.map(([_, count]) => count),
          backgroundColor: [
            "rgba(255,99,132,0.7)",
            "rgba(255,159,64,0.7)",
            "rgba(255,205,86,0.7)",
    ...
    new Chart(ctx2, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: "rgba(220,105,85,0.7)",
          borderColor: "rgba(220,105,85,1)",
          borderWidth: 1
        }]
      },
      options: {
